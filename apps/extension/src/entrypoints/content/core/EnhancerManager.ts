import { detectPlatform, PlatformConfig } from "../config/platforms";
import { EnhancementState } from "../types";
import {
  injectUIElement,
  isUIElementInjected,
  removeUIElement,
...
export class EnhancerManager {
  ...
  private async waitForPlatformElements(): Promise<void> {
    ...
    const editor = await waitForElement(this.platform.selectors.editor, 10000);
    if (!editor) {
      throw new Error(
  ...
  private async injectUI(): Promise<void> {
    ...
    const result = injectUIElement(container, this.platform, this.ENHANCER_ID);
   ...
   */
  private handleEnhancement(originalText: string, enhancedText: string): void {
